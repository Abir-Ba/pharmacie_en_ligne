...
exports.find = function (req, res) {
  Pool.getConnection(function (err, connection) {
    ...
    connection.query('SELECT * FROM Medicament WHERE Nom LIKE ? OR Dosage LIKE ? ', ['%' + searchTerm + '%', '%' + searchTerm + '%'], function (err, rows) {
      connection.release();
...
exports.create = function (req, res) {
  ...
  Pool.getConnection(function (err, connection) {
    ...
    console.log('connected as id ' + connection.threadId);
    var searchTerm = req.body.search;
    connection.query('INSERT INTO Medicament SET Nom = ? , Prix = ?, Dosage = ? , DE = ?, DF = ?, Description = ?  ', [nom, prix, dosage, de, df, description], function (err, rows) {
      connection.release();
...
exports.update = function (req, res) {
  ...
  Pool.getConnection(function (err, connection) {
    if (err) throw err;
    console.log('connected as id ' + connection.threadId);
    connection.query('UPDATE Medicament SET Nom = ? , Prix = ?, Dosage = ? , DE = ?, DF = ?, Description = ? WHERE idMedicament = ?', [nom, prix, dosage, de, df, description, req.params.id], function (err, rows) {
      connection.release();
